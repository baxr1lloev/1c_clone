...
export default function SalesOrdersPage() {
  ...
  const postMutation = useMutation({
    mutationFn: async (id: number) =>
      api.post(`/documents/sales-orders/${id}/post/`),
    onSuccess: () => {
  ...
  const unpostMutation = useMutation({
    mutationFn: async (id: number) =>
      api.post(`/documents/sales-orders/${id}/unpost/`),
    onSuccess: () => {
  ...
  const handleEdit = (doc: SalesOrder) =>
    router.push(`/documents/sales-orders/${doc.id}`);

  const mainActions: CommandBarAction[] = [
    {
      label: t("salesOrders.actions.createOrder"),
      icon: <PiPencilBold />,
  ...
  const columns: ColumnDef<SalesOrder>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <Badge variant={variant}>{t(val)}</Badge>;
      },
    },
  ];
