...
import type { CashOrder, PaginatedResponse, DocumentStatus, CashOrderType } from '@/types';
...
export default function CashOrdersPage() {
    ...
    const { data, isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            const response = await api.get<PaginatedResponse<CashOrder>>('/documents/cash-orders/');
    ...
    const postMutation = useMutation({
        mutationFn: async (id: number) => api.post(`/documents/cash-orders/${id}/post_document/`),
        onSuccess: () => {
            toast.success(t('document_posted'));
            queryClient.invalidateQueries({ queryKey: ['cash-orders'] });
        },
        onError: (err: any) => toast.error(err.response?.data?.error || t('post_failed')),
    });

    const unpostMutation = useMutation({
        mutationFn: async (id: number) => api.post(`/documents/cash-orders/${id}/unpost_document/`),
        onSuccess: () => {
            toast.success(t('document_unposted'));
            queryClient.invalidateQueries({ queryKey: ['cash-orders'] });
        },
        onError: (err: any) => toast.error(err.response?.data?.error || t('unpost_failed')),
    });
    ...
    // Navigation
    const handleCreate = (type: string) => router.push(`/documents/cash-orders/new?type=${type}`);
    const handleEdit = (doc: CashOrder) => router.push(`/documents/cash-orders/${doc.id}/edit`);
    const handleView = (doc: CashOrder) => router.push(`/documents/cash-orders/${doc.id}`);
    ...
    const columns: ColumnDef<CashOrder>[] = [
        {
            ...
            cell: ({ row }) => {
                ...
                return <span className="font-mono text-xs text-muted-foreground">{d.toLocaleDateString()}</span>
            },
        },
        {
            accessorKey: 'number',
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className={cn("flex items-center gap-2 text-xs font-medium", isIncoming ? "text-emerald-600" : "text-amber-600")}>
        ...
        },
        {
            ...
            cell: ({ row }) => <span className="text-xs truncate max-w-[200px] block" title={row.getValue('purpose')}>{row.getValue('purpose')}</span>,
        },
        {
            accessorKey: 'status',
            header: tc('status'),
    ...
    const selectionActions: CommandBarAction[] = selectedItem ? [
        ...
        },
        {
            ...
            disabled: !selectedItem.can_edit && selectedItem.status !== 'cancelled', // Can usually delete drafts
            variant: 'destructive',
            shortcut: 'Del'
        }
    ] : [];
        ...
        <div className="flex flex-col h-[calc(100vh-4rem)]">
            <div className="flex items-center justify-between border-b px-6 py-4 bg-background">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-bold tracking-tight">Кассовые ордера</h1>
                        <p className="text-sm text-muted-foreground">Cash Orders (PKO / RKO)</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col p-6 pt-4 space-y-4">
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList>
                        ...
                        <TabsTrigger value="incoming">Incoming (PKO)</TabsTrigger>
                        <TabsTrigger value="outgoing">Outgoing (RKO)</TabsTrigger>
                    </TabsList>
                </Tabs>
                ...
                <div className="flex-1 border rounded-md bg-card overflow-hidden flex flex-col">
                    <DataTable
                        ...
                        onRowDoubleClick={(row) => row.can_edit ? handleEdit(row) : handleView(row)}
                        ...
                        commandBar={
                            <div className="flex items-center justify-between w-full p-2 border-b bg-muted/20">
                                <CommandBar
                                    ...
                                    searchPlaceholder="Search number, counterparty..."
                                />
            ...
            <AlertDialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>{tc('delete_confirmation')}</AlertDialogTitle>
