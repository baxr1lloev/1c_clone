...
import { FAReceiptDocument, PaginatedResponse, DocumentStatus } from '@/types';
...
const statusColors: Record<DocumentStatus, string> = {
    ...
    cancelled: 'bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-200', // cancelled not used in FA yet?
};

export default function FAReceiptsPage() {
    ...
    const [selectedItem, setSelectedItem] = useState<FAReceiptDocument | null>(null);
    ...
    const handleCreate = () => router.push('/documents/fa-receipts/new');
    const handleEdit = (doc: FAReceiptDocument) => router.push(`/documents/fa-receipts/${doc.id}`);
    ...
    // Columns
    const columns: ColumnDef<FAReceiptDocument>[] = [
        {
            accessorKey: 'date',
            header: ({ column }) => (
