...
export function Sidebar() {
  ...
  const NavItemRenderer = ({
    ...
    if (sidebarCollapsed && depth === 0) {
      return (
        <Link href={item.href} title={t(`${item.id}`)}>
          <Button
      ...
      <Link href={item.href}>
        <Button
          ...
          className={cn(
            ...
            depth > 0 && "pl-8 text-muted-foreground",
          )}
        >
          <Icon className="h-4 w-4 shrink-0" />
                ...
                <div key={group.id} className="space-y-1">
                  {!sidebarCollapsed ? (
                    ...
                    <div className="flex flex-col items-center gap-1">
                      <div className="p-2" title={t(`${group.id}`)}>
              ...
              <CommandGroup key={group.id} heading={t(`${group.id}`)}>
                {group.items.map((item) => (
                  <Fragment key={item.href}>
                    <CommandItem
                      value={`${t(`${item.id}`)} ${item.keywords?.join(" ") || ""}`}
                      onSelect={() => {
