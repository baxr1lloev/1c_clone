  ...
  return (
    <div className="flex items-center gap-2">
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="sm" className="h-8 px-2 text-xs">
            ...
            {defaultView ? defaultView.name : tc("views")}
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="w-56">
          <DropdownMenuLabel>{tc("views")}</DropdownMenuLabel>
          <DropdownMenuSeparator />
            ...
            views.map((view) => (
              ...
              >
                <span className="flex items-center gap-2">
                  {view.isDefault && (
                    <PiStarBold className="h-3 w-3 text-yellow-500" />
                  )}
                  {view.name}
                </span>
                <div
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{tc("save")}</DialogTitle>
            <DialogDescription>
              Save your current filters, sorting, columns, and grouping
              settings.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="view-name">{tc("name")}</Label>
              <Input
                id="view-name"
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveView}>{tc("save")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
